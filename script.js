...
function activeLink() {
    navLinks.forEach(item => item.classList.remove('active')) // Remove a class "active" de todos os links
    this.classList.add ('active'); // Adicona a classe "active ao link clicado"
}
...
// Dunção para alternar entre os temas claro e escro
function toggleMode() {
...
function animateText() {
    if ( isTyping) {
        ...
        if (index > 1) {
            ...
            currentColor = curretColor === (document.documentElement.classList.contains ('light') ? 'black' : '#fff') ? '#C94C16' : (document.documentElement.classList.contains('light') ? 'black' : '#fff');
...
// Função para avançar o próximo slide
function nextSlide() {
...
// Seleciona o formulário de contrato e a mensagem de agradecimento
const contactForm = document.getElementById('contatForm');
const thankYouMessage = document.getElementById('thankYouMessage');

// Adiciona um evento ed envio ao formulário
...
function checkAboutVisibility() {
    ...
    const windowHeight = window.innerHeight || document.documentElement.clienteHeight;
